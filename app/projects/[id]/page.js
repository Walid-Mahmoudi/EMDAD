...
export default function ProjectWorkspace() {
  ...
  async function addActivity(e) {
    ...
    const { error } = await supabase.from('activities').insert({ ...activity, project_id:id, company_id:project.company_id, employee_id:emp?.id });
  ...
  async function addCollection(e) {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    const { data: emp } = await supabase.from('employees').select('id').eq('user_id', user?.id).single();
    const { error } = await supabase.from('collections').insert({ ...collection, project_id:id, company_id:project.company_id, employee_id:emp?.id });
  ...
  return <main className="p-6 space-y-6">
    ...
    <section className="grid gap-6 lg:grid-cols-[2fr_1fr]">
      ...
      <aside className="space-y-4"><Card title="Owner" value={project.employees?.full_name||'Unassigned'}/><Card title="Company" value={project.companies?.company_name||'—'}/><Card title="Contract No." value={project.contract_number||'—'}/><Card title="Location" value={project.location||'—'}/></aside>
    </section>
    <section className="grid gap-6 lg:grid-cols-2">
      <form onSubmit={addActivity} className="rounded-xl border bg-white p-5 space-y-3"><h2 className="text-xl font-semibold">Add Activity / Follow-up</h2><select className="w-full rounded border p-2" value={activity.activity_type} onChange={e=>setActivity({...activity,activity_type:e.target.value})}>{activityTypes.map(x=><option key={x}>{x}</option>)}</select><input className="w-full rounded border p-2" type="datetime-local" value={activity.activity_date} onChange={e=>setActivity({...activity,activity_date:e.target.value})} required/><textarea className="w-full rounded border p-2" placeholder="Notes" value={activity.notes} onChange={e=>setActivity({...activity,notes:e.target.value})}/><input className="w-full rounded border p-2" type="datetime-local" value={activity.next_follow_up_date} onChange={e=>setActivity({...activity,next_follow_up_date:e.target.value})}/><input className="w-full rounded border p-2" placeholder="Next action" value={activity.next_action} onChange={e=>setActivity({...activity,next_action:e.target.value})}/><button disabled={saving} className="rounded-lg border px-4 py-2 font-medium">{saving?'Saving...':'Add Activity'}</button></form>
