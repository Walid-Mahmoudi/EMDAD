...
export default function CRMEnhancements(){
 ...
 return <>
  {projectsPage&&<button className="probability-fab project-manager-fab" onClick={()=>{setOpen(true);setMode('projects');load()}} title="Manage projects">Manage Projects</button>}
  {open&&<div className="probability-overlay" onMouseDown={e=>e.target===e.currentTarget&&setOpen(false)}><div className="probability-panel crm-control-panel">
   <div className="probability-head"><div><small>PROJECT MANAGEMENT</small><h3>Edit projects</h3><p>Update project value, probability, temperature, stage and next action.</p></div><button onClick={()=>setOpen(false)}>×</button></div>
