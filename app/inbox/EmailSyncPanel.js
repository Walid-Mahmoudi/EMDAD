...
export default function EmailSyncPanel(){
  ...
  return <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:14,margin:'0 0 16px',padding:'13px 15px',border:'1px solid #dfe6ef',borderRadius:12,background:'#fff'}}>
    <div style={{display:'flex',alignItems:'center',gap:10,minWidth:0}}>
      ...
      <div style={{minWidth:0}}><b style={{display:'block',fontSize:13,color:'#1a2941'}}>Email intake</b><span style={{display:'block',fontSize:11,color:'#7a879a',marginTop:2}}>Read new messages from mail.emdad.net and add them to the inbox.</span></div>
    </div>
    ...
    {result&&<div style={{display:'flex',alignItems:'center',gap:7,color:'#087443',fontSize:11,fontWeight:700}}><CheckCircle2 size={15}/> {result.inserted} new</div>}
