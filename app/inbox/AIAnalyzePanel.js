...
export default function AIAnalyzePanel(){
  ...
  async function analyze(){
    ...
    try{
      ...
      const firstFailure=(data.results||[]).find(x=>!x.ok);
      setMessage(`Analyzed ${data.analyzed} · Failed ${data.failed}`);
      if(firstFailure?.error) setError(`First failure: ${firstFailure.error}`);
  ...
  return <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:14,margin:'0 0 16px',padding:'13px 15px',border:'1px solid #dfe6ef',borderRadius:12,background:'#fff',flexWrap:'wrap'}}>
    ...
    {error&&<div style={{display:'flex',alignItems:'center',gap:7,color:'#b42318',fontSize:11,fontWeight:700,flex:'1 1 100%'}}><AlertTriangle size={15}/><span style={{wordBreak:'break-word'}}>{error}</span></div>}
